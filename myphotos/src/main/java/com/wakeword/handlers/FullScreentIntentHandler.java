...
public class FullScreentIntentHandler implements RequestHandler {

    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName(Constants.FULL_SCREEN_INTENT));
    }
    public Optional<Response> handle(HandlerInput input) {
       	if (ISPUtil.hasPremiumAccess(input)) {	
		     ...
		     if (sessionAttributes.containsKey("IMAGE_UUID_LIST")) {
		    	 imageUUID = sessionAttributes.get("SESSION_SELECTED_IMAGE_UUID").toString(); 
		     }
			 ...
			 if (AplUtil.supportsApl(input)) {
		          try {
		              ...
		              if (Integer.parseInt(media.getMediaMetadata().getWidth()) > Integer.parseInt(media.getMediaMetadata().getHeight())) {
		            	  document = mapper.readValue(new File("apl_image_full_view_wide_template.json"), documentMapType);
		              } else {
		            	  document = mapper.readValue(new File("apl_image_full_view_template.json"), documentMapType);
		              }
