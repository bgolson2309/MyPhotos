...
public class PreviousIntentHandler implements RequestHandler {
    ...
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName(Constants.AMAZON_PREVIOUS_INTENT));
    }
    public Optional<Response> handle(HandlerInput input) {
     ...
     if (sessionAttributes.containsKey("IMAGE_UUID_LIST")) {
    	 imageUUID = StringUtils.getNextImageUUID("PrevButton", sessionAttributes.get("IMAGE_UUID_LIST").toString(), sessionAttributes.get("SESSION_SELECTED_IMAGE_UUID").toString()); 
     }
